...
class Service {
  ...
  _find (query, paginate, consistency = null) {
    ...
    return this.schema()
      .then(bucket => bucket
        .queryAsync(this._buildN1QL(couchbase, queryStr, consistency, true), values)
        ...
        .then(
          _obj => this.schema()
            .then(bucket => bucket
              ...
              .replaceAsync(this._key(id), Object.assign(Object.assign({
                [this.id]: _obj[this.id]
              }, data), { _type: this.opts.name }))
            )
        )
  ...
  patch (id, data, params) {
    return new Promise((resolve) => {
      console.log(id, params, this._isMultiAllowed('patch'));

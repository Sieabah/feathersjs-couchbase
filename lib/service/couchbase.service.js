...
class Service {
  constructor(opts){
    ...
    if(this.opts.couchbase == null)
      throw new Error('Must pass couchbase library');

    if(this.opts.bucket == null)
      throw new Error('Must pass bucket name');

    if(this.opts.connection == null)
      throw new Error('Must pass bucket connection');

    if(this.opts.name == null)
      throw new Error('Name of service must be specified');
        ...
        .then(([results, queryData]) => {
          if(selected.length > 0)
            results = results.map(a => {
              const result = {};
              for(let key in a)
                if(a.hasOwnProperty(key) && key in selected)
                  result[key] = a[key];

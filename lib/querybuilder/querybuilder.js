...
class QueryBuilder {
  ...
  _where (components, operation = 'AND', { addWhere = true, subQuery = false }) {
    let statements = [];
    ...
    const prefix = (addWhere ? 'WHERE ' : '');
    const statement = statements.join(` ${operation.toUpperCase()} `);

    return prefix + (subQuery ? ['(', ')'].join(statement) : statement);
  }
  ...
  _buildWhere (sub, comparator='AND') {
    ...
    const stringify = (part) => {
      if(part.value != null)
        part.value = this._parameterValue(part.value);
    ...
    for(let part of sub) {
      if (~['and', 'or'].indexOf(part.directive.type)) {

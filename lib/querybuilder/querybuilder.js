...
class QueryBuilder {
        ...
        switch (bareField) {
          case 'sort':
            ...
            for (let key in $query[field])
              if ($query[field].hasOwnProperty(key))
  ...
  _select(components){
    ...
    if(this.bucket != null)
      components = components.map((component) => `\`${this.bucket}\`.${component}`);
  ...
  build(){
    ...
    if(this.bucket)
      $query.push(this._from(this.bucket));

    if($where.length > 0)
      $query.push(this._where($where));

    if($sort.length > 0)
      $query.push(this._sort($sort));

    if($limit != null)
      $query.push(this._limit($limit));

    if($skip != null)
      $query.push(this._skip($skip));

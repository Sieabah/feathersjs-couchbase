...
class QueryBuilder {
  ...
  limit (amount) {
    if (amount == null) { throw new QueryError('Amount must be specified'); }
  ...
  sort (fields, order = 'ASC') {
    if (fields == null) { throw new QueryError('Required to have at least one field to sort'); }

    if (typeof fields === 'string') { fields = [fields]; }

    if (!~['ASC', 'DESC'].indexOf(order.toUpperCase())) { throw new QueryError('Order must be ASC or DESC'); }
  ...
  interpret ($query, fieldName = null) {
    ...
    for (let field in $query) {
      ...
      if (field[0] === '$' && ~whereClauses.indexOf(bareField)) {
        ...
        if (fieldName != null) { this.where(fieldName, bareField, $query[field]); } else { this.where(null, bareField, $query[field]); }
      }
      // Handle equality
      else if (field[0] !== '$' && typeof $query[field] !== 'object') {
 ...
 if ($query[field].hasOwnProperty(key)) { this.sort(key, $query[field][key] > 0 ? 'ASC' : 'DESC'); }
}
            break;
          default:
  ...
  _select (components) {
    ...
    if (this.bucket != null) { components = components.map((component) => `\`${this.bucket}\`.${component}`); }
  ...
  _where (components, operation = 'AND') {
    ...
    for (let component of components) {
      ...
      if (value == null) { value = 'NULL'; } else { value = this._parameterValue(value); }
  ...
  build () {
    ...
    for (let component of this.query) {
      switch (component.type) {
        ...
        // query.$limit -> LIMIT
        case 'limit':
          if (!isNumeric(component.params)) { throw new QueryError('Limit parameter must be numeric'); }

          $limit = parseInt(component.params);
          break;
        // query.$skip -> OFFSET
        case 'skip':
          if (!isNumeric(component.params)) { throw new QueryError('Skip parameter must be numeric'); }
    ...
    $query.push(this._select($select));

    if (this.bucket) { $query.push(this._from(this.bucket)); }
